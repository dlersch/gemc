...
map<string, double> dc_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
    ...
    double rotate_to_sec = (-60*SECI)*deg_to_rad;
    
    int sl_sign; //check superlayer and define sign
    for(int i=0;i<3;i++){
		 if(SLI == 2*i+1){
			  sl_sign = -1; 
		 }else sl_sign = 1;
    ...
    double rotate_to_wire = 6*sl_sign*deg_to_rad;
    double const2 = sl_sign*sin(6*deg_to_rad);
    double const1= cos(6*deg_to_rad);
    
    
    G4ThreeVector vec1,vec2,vec3,vec4;
	...
	for(unsigned int s=0; s<nsteps; s++)
	{
		...
		if(DOCA.mag() <= doca && stepTrackId[s] == trackIds )
		{
			...
			vec1 = G4ThreeVector(mom[s].x(),mom[s].y(),mom[s].z());
			vec2 = G4ThreeVector(mom[s].x(),mom[s].y(),mom[s].z());
			vec3 = G4ThreeVector(mom[s].x(),mom[s].y(),mom[s].z());
			vec4 = G4ThreeVector(mom[s].x(),mom[s].y(),mom[s].z());
			
			//First, rotate px and py to the sector,that has fired:
			vec1.rotateZ(rotate_to_sec);
			vec2.rotateZ(rotate_to_sec);
			vec3.rotateZ(rotate_to_sec);
			vec4.rotateZ(rotate_to_sec);
			
			//Secondly, rotate px' and pz towards the firing sector:
			vec1.rotateY(25*deg_to_rad);
			vec2.rotateY(-25*deg_to_rad);
			vec3.rotateY(25*deg_to_rad);
			vec4.rotateY(-25*deg_to_rad);
			
			//Finally, rotate px'' and py' parallel to the wires:
			vec1.rotateZ(rotate_to_wire);
			vec2.rotateZ(rotate_to_wire);
			vec3.rotateZ(-rotate_to_wire);
			vec4.rotateZ(-rotate_to_wire);
			
			alpha1 = asin((const1*vec1.x() + const2*vec1.y())/vec1.mag())/deg_to_rad;
			alpha2 = asin((const1*vec2.x() + const2*vec2.y())/vec1.mag())/deg_to_rad;
			alpha3 = asin((const1*vec3.x() + const2*vec3.y())/vec1.mag())/deg_to_rad;
			alpha4 = asin((const1*vec4.x() + const2*vec4.y())/vec1.mag())/deg_to_rad;
    ...
    //cout <<"Magnetic field: "<<thisMgnf<< endl;
	...
	//calculate some tdcs here:
	int new_tdc1,new_tdc2,new_tdc3,new_tdc4;
	double time_unsm1,time_unsm2,time_unsm3,time_unsm4;
	
	time_unsm1 = calc_Time(doca/cm,dcc.dmaxsuperlayer[SLI],dcc.tmaxsuperlayer[SECI][SLI],alpha1,thisMgnf,SECI,SLI);
    time_unsm2 = calc_Time(doca/cm,dcc.dmaxsuperlayer[SLI],dcc.tmaxsuperlayer[SECI][SLI],alpha2,thisMgnf,SECI,SLI);
    time_unsm3 = calc_Time(doca/cm,dcc.dmaxsuperlayer[SLI],dcc.tmaxsuperlayer[SECI][SLI],alpha3,thisMgnf,SECI,SLI);
    time_unsm4 = calc_Time(doca/cm,dcc.dmaxsuperlayer[SLI],dcc.tmaxsuperlayer[SECI][SLI],alpha4,thisMgnf,SECI,SLI);

    double time_sm1,time_sm2,time_sm3,time_sm4;
    time_sm1 = fabs(CLHEP::RandGauss::shoot(time_unsm1, smearF*dcc.smearScale[SECI][SLI]));
    time_sm2 = fabs(CLHEP::RandGauss::shoot(time_unsm2, smearF*dcc.smearScale[SECI][SLI]));
    time_sm3 = fabs(CLHEP::RandGauss::shoot(time_unsm3, smearF*dcc.smearScale[SECI][SLI]));
    time_sm4 = fabs(CLHEP::RandGauss::shoot(time_unsm4, smearF*dcc.smearScale[SECI][SLI]));
	...
	//dgtz["wire"]       = nwire;
	//dgtz["LR"]         = LR;
	dgtz["sector"]     = new_tdc2;
	//dgtz["superlayer"] = new_tdc2;
	dgtz["wire"]       = new_tdc3;
	dgtz["LR"]         = new_tdc4;
